...
@Composable
fun SignUpScreen(navController: NavHostController, modifier: Modifier, viewModel: ViewModel) {
    ...
    var eyeOpener by remember { mutableStateOf(false) }
        Column(
            ...
            Button(onClick = {
                if(viewModel.name.isNotEmpty() && viewModel.email.isNotEmpty() &&  viewModel.password.isNotEmpty()){
                    viewModel.sign(viewModel.name,viewModel.email,viewModel.password)
                    viewModel.name=""
                    viewModel.email=""
                    viewModel.password=""
                }else{
